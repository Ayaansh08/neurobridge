...
# NeuroBridge — Standing Rules for AI Agents

_Read this file, `docs/TASKS.md`, and `docs/ARCHITECTURE.md` before any task._

## 1. READ FIRST
Read `AGENTS.md`, `docs/TASKS.md`, and `docs/ARCHITECTURE.md` before starting any task.

## 2. PLAN FIRST
Give 3–5 plain-English bullets before coding. **WAIT for approval** on:
- Any AWS-touching command (`cdk deploy`/`destroy`, `npm run seed`, `aws` CLI)
- New AWS services or new npm dependencies
- Deleting files, auth changes, DynamoDB table definition changes

## 3. AWS SAFETY
Before any AWS command, state the account, region, and stack it targets. Only one person deploys at a time. **Never** run `cdk destroy` unless explicitly asked.

## 4. SECRETS
Never commit secrets, keys, or `.env` files. Never hardcode credentials. `VITE_` vars are public browser code — **never** put a secret in one. Secrets go server-side (Lambda env vars, Secrets Manager, SSM). Warn immediately if you spot a secret in the repo.

## 5. DOCS STAY IN SYNC
After every change, update `docs/TASKS.md` and `docs/CHANGELOG.md`, plus any affected doc (`ARCHITECTURE`, `API_AND_DATA`, `AWS_RESOURCES`, `SETUP`, `DECISIONS`, `KNOWN_ISSUES`, `GLOSSARY`) in the same commit.

## 6. CONTRACT
Frontend and backend agree via `docs/API_AND_DATA.md`. If you change an API shape, update the doc + both sides + say who it may break.

## 7. GIT
Feature branches only. Small focused commits with clear messages. Never push to `main`, never force-push, never rewrite history.

## 8. VERIFY
Before saying "done", run `npm run lint` and `npm run build` and report results. Say how to test manually and what "working" looks like.

## 9. README IS A CLAIM, NOT TRUTH
Check it against the code. Never invent APIs, libraries, or AWS features. If unsure, say so and check official docs.

## 10. COST
Flag anything that could cost real money (NAT gateways, provisioned resources, large model usage). Prefer serverless and free tier. State cost impact before deploying.

## 11. USER DATA
Sessions hold personal practice conversations. Never log message contents or emails to console/CloudWatch. Never use real personal data in seeds, tests, or docs.

## 12. PARALLEL AGENTS
Only edit files in your assigned area. Mark what you're working on in `docs/TASKS.md`. Use your own branch.

## 13. SMALL SCOPE
One feature or fix at a time. No unrequested refactors. If a requirement is ambiguous, ask ONE clear question.
